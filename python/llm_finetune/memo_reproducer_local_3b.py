...
# from torch.distributed._composable.fsdp import fully_shard  # Optional
...
def get_xpu_memory_used_from_xpu_smi(tag, device_id=0):
    ...
    try:
        ...
        total_mem_mb = sum(
            metric["value"] for tile in tile_level for metric in tile.get("data_list", [])
    ...
    for epoch in range(1):
        for step, batch in enumerate(dataloader):
            ...
            memory_log = (
                ...
                if device_type == "xpu"
                else get_cuda_memory_used_from_nvidia_smi(f"Epoch {epoch}, Step {step}", device.index)
            )
