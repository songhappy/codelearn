...
class TrainSpec:
    ...
    flash_attention: bool = True  # attention bytes via AI model
...
def bytes_forward_per_token(spec: LlamaSpec, train: TrainSpec, hw: HardwareSpec) -> float:
    """
    Non-attention GEMMs use IO lower bound:
      bytes ≈ 2·(MK + KN + MN)·b  ; per token -> divide by M_micro = B·S.
    Attention bytes via arithmetic intensity AI:
    ...
    S, b = train.seq_len, hw.bytes_per_el
    ...
    # attention bytes via AI
    F_attn_token = L * (4 * D * S)
    AI = (S/2.0) if train.flash_attention else (82.0 + 0.005*S)
    attn_bytes = F_attn_token / AI

    # LM head bytes
    lm = gemm_bytes_token(M_micro, D, V, b)
    return nonattn + attn_bytes + lm

def bytes_train_per_step(spec: LlamaSpec, train: TrainSpec, hw: HardwareSpec) -> float:
    alpha = 4 if train.checkpoint else 3
    return alpha * bytes_forward_per_token(spec, train, hw) * train.tokens_per_step

def adam_bytes_per_step(spec: LlamaSpec, bf16_states: bool = True) -> int:
    """
    Optimizer bytes/param/step (bf16 states):
      read {g,p,m,v} = 8B, write {p,m,v} = 6B ⇒ ~14 B/param/step (bf16)
      (If FP32 moments, ~28 B/param/step)
    """
    per_param = 14 if bf16_states else 28
    return per_param * spec.total_params

# ---------- roofline timing ----------
def roofline_step(spec: LlamaSpec, train: TrainSpec, hw: HardwareSpec,
                  adam_variant: str = "adamw", bf16_states: bool = True) -> Dict[str, float]:
    # FLOPs
    ...
    # BYTES
    B_train_step = bytes_train_per_step(spec, train, hw)
    B_adam_step  = adam_bytes_per_step(spec, bf16_states=bf16_states)
...
# ---------- memory footprint ----------
def memory_persistent_bytes(spec: LlamaSpec, bf16_states: bool = True) -> int:
    """
    Persistent model state (bf16):
      params (2B) + grads (2B) + m (2B or 4B) + v (2B or 4B)
    """
    P = spec.total_params
    param_b = 2*P
    grad_b  = 2*P
    mv_b    = (2 if bf16_states else 4) * P * 2
    return param_b + grad_b + mv_b

def memory_activation_bounds(spec: LlamaSpec, train: TrainSpec, bytes_per_el: int = 2) -> Tuple[int, int]:
    """
    Very rough bracket:
      lower (ckpt + FA): ~ L · (B·S) · (2D) · bytes
      upper (no-ckpt):   ~ L · (B·S) · (3D + 2D_ff + 2D) · bytes
    """
    M_micro = train.batch_size * train.seq_len
...
def print_out(spec, hw, train):
    ...
    # Bytes and roofline
    b_fwd_tok = bytes_forward_per_token(spec, train, hw)
    B_trn = bytes_train_per_step(spec, train, hw)
    B_opt = adam_bytes_per_step(spec, bf16_states=True)
    res = roofline_step(spec, train, hw, adam_variant="adam", bf16_states=True)
    ...
    # Memory footprint
    persist = memory_persistent_bytes(spec, bf16_states=True)
    act_lo, act_hi = memory_activation_bounds(spec, train, hw.bytes_per_el)
    print("\n=== Memory footprint ===")
...
if __name__ == "__main__":
    ...
    # Hardware: B = 920e9 * 0.9 bytes/s; T = 1066e12 * 1 FLOPs/s; bf16 (2B/elt)
    hw_1550 = HardwareSpec(
        bandwidth_Bps_raw=1052e9, bandwidth_eff=1.0,
        compute_Fps_raw=104.86e12, compute_eff=0.9,
        bytes_per_el=2
    )
